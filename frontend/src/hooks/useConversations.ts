...
function mapSummaryToChat(s: ConversationSummary): Chat {
  // Backend already chooses the correct display name:
  // - If a group has a custom title, friendName is that title.
  // - If it has no title, friendName is derived from participants.
  const displayName = s.friendName ?? "";
...
export function useConversations(initial: Chat[] = []) {
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      ...
      const mapped = data.map((s) => mapSummaryToChat(s));
